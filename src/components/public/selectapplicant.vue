<template>
    <div class="selectapplicantmsgbox">
        ...
        <div class="caozuobtns">
            <span class="qx" @click="quxiaocz">取消</span><span class="queren" @click="querencz">确定</span>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        querencz() {
            ...
            if(!this.wts || !this.wts_oss) {
                this.$message.warning('请上传委托书')
                return
            }
            const senddata = {
                entrustFile: this.wts,
                entrustFileOsskey: this.wts_oss,
                applicantTbId: this.current
            }
            this.$emit('selectedapplicant', senddata)
        },
        downloadwts() {
            if(!this.current) {
