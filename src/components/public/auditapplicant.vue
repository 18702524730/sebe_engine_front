<template>
	<div id="auditapplicant">
		<div class="applicantbox">
			<h3 class="addtitle">
				查看申请人 <i class="el-icon-close" @click="close"></i>
			</h3>
            <div class="contentbox">
                <!-- <div class="unclickbox" v-if="unclickshow"></div> -->
                <div class="clearfix mb20">
                    <div class="leftpart">
                        ...
                        >
                            ...
                            <el-form-item label="邮编" required key="postcode">
                                <el-input
                                    placeholder="请输入邮编号码"
                                    v-model="postcode"
                                ></el-input>
            ...
            <div class="submitbox">
                <span class="delbtn" @click="delitem">删除</span>
                <span class="submitbtn" @click="createapp">提交</span>
                <!-- <span class="delbtn" @click="close" v-if="unclickshow">取消</span>
                <span class="submitbtn" @click="unclickshow=false" v-if="unclickshow">修改</span> -->
            </div>
		</div>
        <el-dialog
...
const delUrl = '/api/deleteApplicant'
        ...
        ApplicantRegion() {
            ...
            if (this.ApplicantType == 2 ) {
                if (this.ApplicantRegion == 1) {
                    ...
                    this.rules = Object.assign({}, company0Rules)
                }
            }
            setTimeout(() => {
                this.$refs.applicantform.resetFields()
	...
	methods: {
        ...
        getdetail() {
            ...
            this.$http.get(detailUrl, { params: data }).then(res => {
                ...
                // this.form.EName = res.data.ename
                // this.form.EAddress = res.data.eaddress
                this.form = {
                    ApplicantName: ret.ApplicantName,
                    CardNumber: ret.CardNumber,
                    Address: ret.Address,
                    EName: ret.Ename,
                    EAddress: ret.Eaddress,
                    Country: ret.Country
                }
                this.ApplicantType = ret.ApplicantType + ''
                this.ApplicantRegion = ret.ApplicantRegion + ''
                if (ret.ApplicantType == 1) {
                    if (ret.ApplicantRegion == 1) {
                        this.province = ret['Province']
                    }
                }
                if (ret.ApplicantType == 2) {
                    if (ret.ApplicantRegion == 1) {
                        this.provincedl = ret['Province']
                    } else {
                        this.provincedc = ret['Province']
                    }
                }
                this.idcard = ret.SebeIdCardOss
                this.idcard_oss = ret.IdCardOssKey
                this.licence = ret.SebeBusinessLicenceOss
                this.licence_oss = ret.BusinessLicenceOssKey
                this.passport = ret.SebePassportOss
                this.passport_oss = ret.PassportOssKey
                this.authorization = ret.SebeAuthorizationOss
                this.authorization_oss = ret.AuthorizationOssKey
                this.legal = ret.SebeLegalNoticeOss
                this.legal_oss = ret.LegalNoticeOssKey
            }).catch(err => {
        ...
        delitem() {
            this.$confirm('是否确认删除？', '提示', {
					...
					this.$http.put(delUrl + "?id=" + this.id).then(res => {
        ...
        setUploadedMaterialcompany(data) {
			...
			if (d.url) {
                ...
                this.postcode = this.postcode || d.postcode
